...
public class OpenVPNThread implements Runnable {
    ...
    // Compatibility shim for the Flutter bridge (VPNHelper.stopVPN(), which
    // predates official's AIDL-bound IOpenVPNServiceInternal.stopVPN()
    // pattern) -- tracks the most recently started instance the same way
    // this fork's pre-migration OpenVPNThread used to.
    private static OpenVPNThread mActiveInstance;

    public static boolean stop() {
        OpenVPNThread active = mActiveInstance;
        if (active != null) {
            if (active.mService != null)
                active.mService.openvpnStopped();
            active.stopProcess();
        }
        return true;
    }
